...
class Personagem 
{
    ...
    public static int compareDates( Personagem perso1, Personagem perso2 )
    {
        int result = 0;
        LocalDate date1 = perso1.getDateOfBirth( );
        LocalDate date2 = perso2.getDateOfBirth( );
        if( date1.isBefore(date2) ) { // d1 vem antes de d2
            result = -1;
        } 
        else if( date1.isAfter(date2) ) { // d1 vem depois de d2 
            result = 1;
        } 
        else // d1 == d2 
        {
    ...
    public static void insertionSort( List<Personagem> perso, Log log )
    {
        for( int i = 1; i < perso.size( ); i = i + 1 ) 
        {
            Personagem key = perso.get(i);
            int j = i - 1;
            while( ( j >= 0 ) && ( compareDates( key, perso.get(j) ) < 0 ) ) 
            {
                perso.set( j+1, perso.get(j) );
                j--;
                log.incrementarComp( ); // segunda condicao do while 
                log.incrementarMov( );
            } // end while
            log.incrementarComp( ); // primeira condicao do while
            perso.set( j+1, key );
        } // end for
    } // end insertionSort ( )
} // end class
...
public class Insercao extends Personagem
{
    public static void main ( String [] args )
    {
        ...
        Log log = new Log( "812839_insercao.txt" );
        Personagem perso = new Personagem( );
